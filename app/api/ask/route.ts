...
const TOPICS:Record<string,string[]>={
 age_dob:["age","date of birth","dob","birth date","year of birth","عمر","تاریخ پیدائش","پیدائش کی تاریخ"],name:["full name","name","نام"],father_name:["father name","father's name","fathers name","father","والد کا نام","والد"],mother_name:["mother name","mother's name","mothers name","mother","والدہ کا نام","والدہ"],address:["address","residential address","پتہ","رہائشی پتہ"],fee:["fee","fees","cost","charges","price","فیس","چارجز"],processing_time:["processing time","how long","working days","delivery time","processing","کتنے دن","کتنا وقت","مدت","پروسیسنگ"],documents:["documents","document","required documents","requirements","papers","کاغذات","دستاویزات","ضروری دستاویزات"],procedure:["procedure","process","apply","application","how to","طریقہ","درخواست"],eligibility:["eligible","eligibility","who can","اہلیت","کون درخواست دے سکتا"],online:["online","pakid","app","website","آن لائن","پاک آئی ڈی"],office:["office","center","centre","location","کہاں","دفتر","مرکز"],renewal:["renew","renewal","تجدید"],lost:["lost","stolen","damaged","گم","چوری","خراب"],status:["status","track","tracking","اسٹیٹس","ٹریک"]};
const SERVICES:Record<string,string[]>={
 "CNIC / NADRA":["cnic","nic","identity card","nadra","شناختی کارڈ","نادرا"],"Passport":["passport","پاسپورٹ"],"Driving Licence":["driving licence","driving license","driving","license","licence","ڈرائیونگ لائسنس","لائسنس"],"Domicile":["domicile","ڈومیسائل"],"Scholarships":["scholarship","scholarships","stipend","financial aid","وظیفہ","اسکالرشپ"],"Protector of Emigrants":["protector","protector of emigrants","emigration","emigrant","overseas employment","work visa","employment visa","پروٹیکٹر","ایمیگریشن","بیرون ملک ملازمت"],"Other Services":["birth certificate","death certificate","marriage certificate","divorce certificate","police verification","vehicle registration","token tax","income tax","fbr","tax","crc","form b","fard","پیدائش","وفات","شادی","طلاق","پولیس ویریفکیشن","گاڑی رجسٹریشن","ٹیکس"]};
...
function detectService(q:string,requested:string):string|null{const text=normalize(q);let best=requested||null,score=requested?5:0;for(const [service,aliases] of Object.entries(SERVICES)){let s=0;for(const a of aliases)if(text.includes(normalize(a)))s+=a.length>=8?15:10;if(s>score){score=s;best=service;}}return best;}
...
function selectRecords(q:string,requested:string,records:VerifiedRecord[]){const topic=detectTopic(q),service=detectService(q,requested),jurisdiction=detectJurisdiction(q);let work=[...records];if(service){const x=work.filter(r=>serviceMatch(r,service));if(x.length)work=x;}if(jurisdiction){const j=normalize(jurisdiction);const x=work.filter(r=>{const p=normalize(r.province);return p==="pakistan"||p.includes(j)||j.includes(p);});if(x.length)work=x;}const scored=work.map(r=>({r,s:topicScore(topic,r)*10+generalScore(q,r)})).sort((a,b)=>b.s-a.s);const topicMatches=topic?scored.filter(x=>topicScore(topic,x.r)>0):[];return{records:(topicMatches.length?topicMatches:scored).slice(0,8).map(x=>x.r),topic,service,jurisdiction};}
...
function sourceForQuestion(question:string,service:string|null,jurisdiction:string|null){const text=normalize(`${question} ${service||""}`);let candidates=OFFICIAL_SOURCES.filter(s=>s.keys.some(k=>text.includes(normalize(k))));if(jurisdiction==="Khyber Pakhtunkhwa"){if(text.includes("driving")||text.includes("license")||text.includes("licence")||text.includes("ڈرائیونگ"))candidates=[{keys:[],url:"https://www.kppolice.gov.pk/",title:"KP Police E-Driving License Services",department:"Khyber Pakhtunkhwa Police"},...candidates];if(text.includes("police")||text.includes("character")||text.includes("verification"))candidates=[{keys:[],url:"https://apipsm.kppolice.gov.pk/psm/VideoTutorial",title:"KP Police Sahulat Markaz",department:"Khyber Pakhtunkhwa Police"},...candidates];if(text.includes("birth")||text.includes("death")||text.includes("marriage")||text.includes("divorce")||text.includes("union council"))candidates=[{keys:[],url:"https://lgkp.gov.pk/page/registration-bdmd",title:"KP Local Government Birth, Death, Marriage & Divorce Registration",department:"Local Government, Elections & Rural Development Department KP"},...candidates];if(text.includes("excise")||text.includes("vehicle")||text.includes("token"))candidates=[{keys:[],url:"https://cfc.kp.gov.pk/Home/",title:"KP Citizens Facilitation Portal",department:"Government of Khyber Pakhtunkhwa"},...candidates];}if(jurisdiction==="Islamabad Capital Territory"&&(text.includes("driving")||text.includes("license")||text.includes("licence")))candidates=[{keys:[],url:"https://dlims.islamabadpolice.gov.pk/",title:"ITP DLIMS Driving Licence Services",department:"Islamabad Traffic Police"},...candidates];if(jurisdiction==="Sindh"&&(text.includes("driving")||text.includes("license")||text.includes("licence")))candidates=[{keys:[],url:"https://dls.gos.pk/",title:"Driving License Sindh",department:"Sindh Police – Driving License Unit"},...candidates];return candidates[0]||null;}
...
export async function POST(request:NextRequest){try{
 ...
 const all=(await db.json()) as VerifiedRecord[];const selected=selectRecords(question,requested,all);const registrySource=sourceForQuestion(question,selected.service,selected.jurisdiction);const recordSource=selected.records.find(r=>r.official_source_url)?.official_source_url||"";const sourceUrl=recordSource||registrySource?.url||"";const sourceMeta=registrySource||{url:sourceUrl,title:selected.records[0]?.official_source_title||"Official Government Source",department:selected.records[0]?.official_department||"Government of Pakistan"};const officialText=sourceUrl?await fetchOfficialPage(sourceUrl):"";const dbContext=selected.records.length?context(selected.records,language):"No matching verified database record was found.";if(!selected.records.length&&!officialText)return NextResponse.json({answer:noInfo(language),source:null});
 const system=`You are the goal-oriented AI agent inside Pakistan Citizen Helper. Your job is to help a citizen reach the practical next step toward completing a government-service goal, not merely provide a generic answer.

Use the verified database records as the primary source. When the database is incomplete, the question depends on current information, or an official source needs checking, use the built-in official web-search tool. Search only official Pakistani government domains relevant to the selected service and province.

Never use your own memory or general web knowledge for government facts. Never invent fees, documents, eligibility, deadlines, office locations, procedures, links, or processing times. If province/jurisdiction matters and is missing, say that the province is needed rather than silently choosing one.

Give the citizen a practical, goal-focused response. When supported, organize it as:
1. Goal understood
2. What to do
3. Documents / requirements
4. Fee and processing time
5. Where or how to apply
6. Next step
Keep it concise and actionable. If the information is unavailable, say clearly that verified information is unavailable. If sources conflict, explain the conflict instead of guessing.

Use simple Pakistani Urdu when the requested language is Urdu. Do not combine unrelated services. Include citations when the web-search tool provides them.

Selected service: ${requested||"not specified"}
Requested language: ${language}
`;
 const ai=await fetch("https://api.groq.com/openai/v1/chat/completions",{method:"POST",headers:{Authorization:`Bearer ${GROQ_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:"groq/compound",temperature:0,max_tokens:1800,citation_options:"enabled",search_settings:{include_domains:webSearchDomains(question,selected.service||requested,selected.jurisdiction)},messages:[{role:"system",content:system},{role:"user",content:`Goal: ${question}\nSelected service: ${selected.service||requested||"not specified"}\nJurisdiction: ${selected.jurisdiction||"not specified"}\nLanguage: ${language}\n\nVERIFIED DATABASE RECORDS:\n${dbContext}\n\nOFFICIAL SOURCE TEXT:\n${officialText||"No official source text was retrieved."}\n\nIMPORTANT: Use the verified records and official source text above. If they do not contain the answer, use browser search to check the relevant official Pakistani government source. Do not invent facts.`}]})});if(!ai.ok){console.error(await ai.text());return NextResponse.json({error:"AI service is temporarily unavailable. Please try again."},{status:500});}const data=await ai.json();const answer=data?.choices?.[0]?.message?.content?.trim()||noInfo(language);return NextResponse.json({answer,source:{department:sourceMeta.department,title:sourceMeta.title,url:sourceUrl,lastVerified:selected.records[0]?.last_verified||"",province:selected.jurisdiction||selected.records[0]?.province||""},agent:true,goalFocused:true,webSearch:true});
