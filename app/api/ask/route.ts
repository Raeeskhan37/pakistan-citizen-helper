...
`;
 ...
 const needsOfficialVerification=selected.topic==="parent_information" || /parent|father|mother|والد|والدہ|والدین/i.test(question);
 const shouldSearch=!hasVerifiedRecords || needsOfficialVerification;
 const messages=[{role:"system",content:system},{role:"user",content:`Goal: ${question}\nSelected service: ${selected.service||requested||"not specified"}\nJurisdiction: ${selected.jurisdiction||"not specified"}\nLanguage: ${language}\n\nVERIFIED DATABASE RECORDS:\n${dbContext}\n\nOFFICIAL SOURCE TEXT:\n${officialText||"No official source text was retrieved."}\n\nIMPORTANT: Use the verified records and official source text above. If they do not contain the answer, use browser search to check the relevant official Pakistani government source. Do not invent facts.`}];
 const aiBody:any={model:"openai/gpt-oss-120b",temperature:1,reasoning_effort:"low",max_completion_tokens:2048,messages};
